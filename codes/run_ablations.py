...
def run_experiment(xp, xp_count, n_experiments):
  ...
  overall_label = [True] * benign_count + [False] * len(attacker_plan)
  ...
  for c_round in range(1, hp["communication_rounds"]+1):
    ...
    if attack_method_set.intersection(["Fang", "Min-Max", "Min-Sum"]):
      mali_clients = []
      flag = False
      for client in participating_clients:
        if getattr(client, "attack_method", "benign") in ["Fang", "Min-Max", "Min-Sum"]:
    ...
    if "REDefense" in hp["aggregation_mode"]:
      ...
      round_iter = 0
      for client in participating_clients:
        if getattr(client, "attack_method", "benign") != "benign":
          labels.append(1)
        else:
          labels.append(0)
      ...
      aggregated_clients = filtered_clients
      if "flame" in hp["aggregation_mode"]:
        aggregated_clients = server.flame(filtered_clients)
      ...
      agg_acc, agg_recall, agg_fpr, agg_fnr, _ = detection_metric_per_round(real_labels, pred_labels)
    ...
    if xp.is_log_round(c_round):
      ...
      xp.log({"agg_dacc": agg_acc, "agg_drecall": agg_recall, "agg_dfpr": agg_fpr, "agg_dfnr": agg_fnr})
